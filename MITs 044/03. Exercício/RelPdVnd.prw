...
/*/{Protheus.doc} User Function RelPdVnd
    3. Exercício | Relatório de Pedido de Venda 
    @type  Function
    @author André Lucas
    @since 18/08/2023
    @version 0.1
...
Static Function TelPerguntas()
    ...
    while nPosPedIni > nPosPedFim
        if Alltrim(MV_PAR02) <> ''
            DbSelectArea('SC5')
            DbSetOrder(1)
            SC5->(DbSeek(xFilial('SC5') + MV_PAR01))
            nPosPedIni := Recno()
            SC5->(DbSeek(xFilial('SC5') + MV_PAR02))
            nPosPedFim := Recno()
            SC5->(DbCloseArea())

            if nPosPedIni > nPosPedFim
                FwAlertError('O "Pedido De" não pode ser posterior ao "Pedido Até"!','Pedido Inválido')
    ...
    while MV_PAR04 < MV_PAR03 .OR. MV_PAR03 > Date() .OR. MV_PAR04 > Date()
        if MV_PAR04 < MV_PAR03
            FwAlertError('A "Data Até" não pode ser Anterior que a "Data De"!','Data Até Inválida')
        elseif MV_PAR03 > Date()
            FwAlertError('A "Data De" não pode ser Futura!','Data De Inválida')
        elseif MV_PAR04 > Date()
            FwAlertError('A "Data Até" não pode ser Futura!','Data Até Inválida')
        endif
        Pergunte(cPergunta, .T.)
    enddo
...
Static Function ConstrCabec(aParam)
    ...
    //? Retorna o Primeiro e Ultimo Pedido
    DbSelectArea('SC5')
    DbSetOrder(1)
    SC5->(DbGoTop())
    cIniPed := SC5->(C5_NUM)
    ...
    if cPedDe <> ''
        if cPedAte <> ''
            ...
            cQuery +=  "AND (SC5.C5_NUM BETWEEN '" + aParam[1] + "' AND '"  + cUltPed + "') " + CRLF
        endif
    else
        if cDataDe == ''
            cQuery +=  "AND (SC5.C5_NUM = '"  + SC5->(C5_NUM) + "') " + CRLF
        endif
    endif
...
Static Function ConstrItens()
    ...
    if cPedidos <> ''
        cQuery += "AND SC6.C6_NUM IN " + cPedidos
    endif
...
Static Function SetInfoRelat(cAlias)
    ...
    oPrint:SayAlign(60, 10, 'Fone: (19) 3851-3100'                                           , oFont14B , (nComprimento - 10) ,,, 1)
    ...
    oPrint:SayAlign(95, 15, 'ANÁLISE CRITICA 2', oFont16  ,  (nComprimento - 15) ,,, 2)

    oPrint:Line(115,15,115,nComprimento,,'-8')
    oPrint:SayAlign(120, 15, 'Pedido N° ' + Alltrim((cAlias)->(C5_NUM)), oFont14  ,  (nComprimento - 15) ,,, 0)
    oPrint:SayAlign(120, 15, 'Data: ' + TRANSFORM(StoD((cAlias)->(C5_EMISSAO)), '@R 99/99/9999'), oFont14  ,  (nComprimento - 15) ,,, 1)
...
Static Function SetCabecPdVnd(cAlias,nComprimento)
    ...
    nQtd := QuebLnInt(cCliente, 36, nColCabec2,oFont12)
...
Static Function QuebLnInt(cString,nQtdCar,nCol,oFont)
...
Static Function CabecRetItens(cAlias)
    ...
    oPrint:SayAlign(nLinha, 37 , 'Cod. Prd.'              , oFont10B ,  33  ,,, 0)
    ...
    oPrint:SayAlign(nLinha, 267, 'Qtd.'/*Pç*/             , oFont10B ,  23  ,,, 0)
    oPrint:SayAlign(nLinha, 292, 'Qtd.'/*Emb.*/           , oFont10B ,  23  ,,, 0)
    ...
    oPrint:SayAlign(nLinha, 552, 'Obs. PCP'               , oFont10B ,  30  ,,, 0)
    nLinha += 10 
    ...
    //? Impressão das Observações do Pedido
...
Static Function CabecPsgItens(cAlias)
    ...
    oPrint:SayAlign(nLinha  , 422, 'Qtd.'/*Emb.*/           , oFont10B ,  33  ,,, 0)
